...
| `6d55071c` | Restore the original phase-and-velocity ignition gate while retaining the early-burn shortcut removal. | **No.** It correctly fired, but its first valid post-burn endpoint was about 185 km from the target. | **No.** The run entered Deceleration Burn with a 129 km predicted miss. | The deorbit calculation rotated the target using a pure periapsis-change candidate, then executed a different redirected burn without validating its actual endpoint. Course Correction then raised periapsis while attempting to recover the large miss. |
| `e102ff6f` | Make the V1 deorbit candidate self-consistent: solve target rotation from the actual redirected burn, retain the accepted inertial aim point during the burn, and refuse ignition outside a body-scaled endpoint corridor. | **No.** It safely refused an invalid deorbit candidate but did not produce a feasible alternative. | **No.** It completed Plane Change then remained in DeorbitBurn at 100× with zero throttle. | Live trace: candidate endpoint error began at 121.5 km and increased; planned direction was nearly 180° from orbital velocity. The angle-derived burn construction cannot serve as the candidate planner. |
