...
| `e102ff6f` | Make the V1 deorbit candidate self-consistent: solve target rotation from the actual redirected burn, retain the accepted inertial aim point during the burn, and refuse ignition outside a body-scaled endpoint corridor. | **Offline checks pass; live result pending.** | Not yet tested. | The candidate solve is rate-limited to one plan per simulated second before ignition to avoid planner-induced frame hitches. |
