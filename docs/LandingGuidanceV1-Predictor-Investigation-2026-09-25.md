...
Terminal translation now derives its lateral component from the negative measured horizontal surface-velocity vector. It seeks a two-second velocity-decay response and limits lateral-to-vertical acceleration to 0.70, approximately 35 degrees of tilt. The throttle controller continues to hold vertical speed using the actual vertical component of thrust, without the generic Landing Guidance minimum-throttle floor. Focused tests cover thrust opposite horizontal velocity, the low-gravity tilt cap, and the zero-horizontal-velocity case.
